...
function draw() {
  ...
  if(stage === 1) {
    ...
    text("Based upon the speed and weight of the car, the deformation (or the damage received) is calculated.",20,100);
    text("~ Any deformation greater than 180 is considered lethal for the passengers.",20,130);
    text("~ Any deformation between 100 and 180 is considered average.",20,160);
    text("~ Deformation below 100 is considered good.",20,190);
  
    text("Press 'r' to start the test.",370,350);
    
   
    

    //Changing stage's value to 2 and playing the start sound when 'r' key is pressed.
    if(keyDown("r")) {
      stage = 2;
      sound.play();
    }
  ...
  //Displaying text according to the deformation value
  if(deformation1 < 100) {
    fill(color(0,255,0));
    textFont("cursive");
